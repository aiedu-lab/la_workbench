...
<!-- AI-GENERATED [claude-code:claude-sonnet-5]: Phase 12 (plan_history.md) -->

## Phase 12: Critical Points

### Step 12.1: Author critical_points.md — Optimization Overview + Single-Variable second derivative test

[x] Status

CONTEXT: `sessions/critical_points.md` does not exist; `.tmp/critical_points_overview.md` holds a full single+multivariable transcript (Optimization Overview, Part I Single Variable: critical points, Taylor expansion, second derivative test) and `.tmp/critical_point_toc.md` gives the target table of contents; this session slots after Partial Derivatives (row 9) and before Column Space, Rank (current row 10) once wired into the Agenda in a later step. ACTION: Create `sessions/critical_points.md` with title "Critical Points and the Second Derivative Test" and a `## Concept` section opening with an "Optimization Overview" subsection (candidates: critical points, boundary points, ±infinity; compare values to find global max/min) including one Mermaid flowchart of that two-stage decision process; then a "Single Variable" subsection adapting `.tmp/critical_points_overview.md` sections 2-4 (critical points via `f'(a)=0`, the Taylor expansion `f(a+h)-f(a) = f'(a)h + (1/2)f''(a)h^2 + ...` with the triangle-area intuition for the 1/2, and the second-derivative test with its three ASCII-art cases: min/max/indeterminate, keeping the ASCII diagrams from the overview file) plus one Mermaid decision flowchart classifying a single-variable critical point (compute `f'(a)` → is it 0? → compute `f''(a)` sign → min/max/indeterminate, needs higher-order check). CONSTRAINTS: Do not yet write the Multivariable subsection, Reference section, or Exercise section (Step 12.2); do not modify `.tmp/critical_points_overview.md` or any other session file; keep ASCII sketches for the geometric bowl/dome pictures and the Taylor triangle, Mermaid only for the two decision-flow diagrams. OUTPUT: New `sessions/critical_points.md` with title, Optimization Overview + Single Variable subsections, and two Mermaid flowcharts. VERIFY: `grep -c "mermaid" sessions/critical_points.md` → `2`; `grep -c "Single Variable" sessions/critical_points.md` → `>0`.

### Step 12.2: Author critical_points.md — Multivariable, n=1 special case, Cheat Sheet, Reference, Exercise

[x] Status

CONTEXT: `sessions/critical_points.md` (Step 12.1) has Optimization Overview + Single Variable content but ends there; `.tmp/critical_points_overview.md` sections 5-13 cover the Multivariable material (gradient, Taylor expansion via `∇f(a)^Tv + (1/2)v^TH(a)v`, directional derivative/Hessian derivation via `g(t)=f(a+tu)`, curvature intuition, positive/negative/indefinite/semidefinite matrices, why eigenvalues matter, Sylvester's Criterion, the multivariable second-derivative test table, and single-variable-as-n=1) plus a Final Summary table; `sessions/partial_derivatives_multivariate_calculus.md` already references MIT 18.02 Lectures 8-12, of which Lecture 9 (max-min problems) and Lecture 10 (second-derivative test) are most relevant here. ACTION: Append to `sessions/critical_points.md`'s `## Concept` section a "Multivariable" subsection adapting overview sections 5-13 (gradient, Taylor expansion, directional derivative, Hessian via the `g(t)=f(a+tu)` derivation, curvature, positive/negative-definite/indefinite/semidefinite with ASCII sketches, eigenvalue reasoning, Sylvester's Criterion, the classification table) with one Mermaid decision flowchart classifying a multivariable critical point (compute `∇f(a)` → is it the zero vector? → compute Hessian `H(a)` → eigenvalues/Sylvester's Criterion → min/max/saddle/indeterminate), then a "Single Variable as n=1" subsection adapting overview section 13, then a "Cheat Sheet" subsection reproducing the overview's Final Summary comparison table; add a `## Reference` section linking the MIT 18.02 video gallery and naming Lecture 9 (max-min problems) and Lecture 10 (second-derivative test) as most directly relevant; add a `## Exercise` section linking `../projects/critical_points/` and naming its two exercises, "Finding the Bowl's Bottom" (single-variable) and "Saddle or Bowl?" (multivariable). CONSTRAINTS: Do not modify the Optimization Overview or Single Variable subsections from Step 12.1; do not modify `.tmp/critical_points_overview.md`, `sessions/partial_derivatives_multivariate_calculus.md`, or any other session file; the `## Exercise` section may name the two project exercises before `projects/critical_points/README.md` exists (created next in Step 12.3). OUTPUT: `sessions/critical_points.md` complete with Concept (all subsections), Reference, and Exercise sections; no TODO markers. VERIFY: `grep -c "mermaid" sessions/critical_points.md` → `3` (2 from Step 12.1 + 1 new multivariable flowchart — the VERIFY count is cumulative over the whole file, not just this step's addition); `grep -c "Sylvester" sessions/critical_points.md` → `>0`; `grep -c "TODO" sessions/critical_points.md` → `0`; `grep -c "## Reference\|## Exercise" sessions/critical_points.md` → `2`.

### Step 12.3: Author projects/critical_points/README.md — two exercises + Help

[x] Status

CONTEXT: No `projects/critical_points/` exists; `sessions/critical_points.md` (Step 12.2) links `../projects/critical_points/` naming exercises "Finding the Bowl's Bottom" and "Saddle or Bowl?"; the MIT 18.02 assignments page has representative problem sets to draw paper problems from; `projects/systems_of_linear_equations/README.md`'s two-named-exercises-plus-trailing-Help structure is the template to follow. ACTION: Create `projects/critical_points/README.md` with a `## Finding the Bowl's Bottom` exercise (single-variable): a paper problem using `f(x) = x^3 - 3*x` asking students to hand-find all critical points via `f'(x)=0`, classify each via `f''(x)`'s sign, and note any indeterminate case; a coding section using `np.roots` on the derivative's coefficients to find critical points, classifying each numerically via a central-difference second derivative, and plotting the curve with each critical point marked and labeled min/max; and a `## Saddle or Bowl?` exercise (multivariable): a paper problem using `f(x, y) = x^2 - y^2` (saddle) and `f(x, y) = x^2 + 2*y^2` (bowl) asking students to hand-compute the gradient, confirm `(0,0)` is a critical point for both, hand-compute the Hessian, and classify each via Sylvester's Criterion; a coding section computing the Hessian, classifying via both `np.linalg.eigvalsh` (eigenvalue signs) and Sylvester's Criterion (leading principal minors) and confirming they agree, then visualizing both surfaces (3D surface or contour) with the critical point marked; a trailing `## Help` section with a `plot_curve_with_critical_points(f, xr, points, labels)` helper (reusing the pattern from `projects/single_variable_calculus/README.md`) and a `plot_surface_with_point(f, xr, yr, point, label)` helper, plus `assert` checks for the single-variable critical points/classifications and for the eigenvalue/Sylvester classification agreement on both the saddle and the bowl. CONSTRAINTS: Keep both exercises toy/basic; do not touch `projects/single_variable_calculus/` or `projects/partial_derivatives_multivariate_calculus/` (Steps 12.5-12.6); do not create a `solutions/` subdirectory. OUTPUT: New `projects/critical_points/README.md` with two named exercises and a Help section. VERIFY: `grep -c "## Help" projects/critical_points/README.md` → `1`; `grep -c "assert" projects/critical_points/README.md` → `>1`; `grep -c "Sylvester" projects/critical_points/README.md` → `>0`.
...
### Step 12.5: Deduplicate Single-Variable Calculus — forward-reference the second-derivative test

[x] Status

CONTEXT: `sessions/single_variable_calculus.md`'s Concept section and `projects/single_variable_calculus/README.md`'s Paper Problem/Coding Exercise/Help each independently classify a critical point via `f''(x)`'s sign — content `sessions/critical_points.md` (Step 12.1) now owns in more depth, including the Taylor-expansion derivation of why the sign of `f''` decides min vs. max. ACTION: In `sessions/single_variable_calculus.md`, trim the paragraph explaining that `f''(x*)`'s sign classifies `x*` as a min or max down to a single sentence noting the classification exists, replaced with a forward reference to [Critical Points and the Second Derivative Test](critical_points.md); in `projects/single_variable_calculus/README.md`, remove the Paper Problem's `f''(x)` sign bullet, the Coding Exercise's numerical-`f''(x*)`-sign bullet, and the Help section's `f''(x_star) > 0` assert, replacing them with a one-line pointer to the "Finding the Bowl's Bottom" exercise in `../critical_points/` for classification practice. CONSTRAINTS: Do not remove the critical-point-finding (`f'(x)=0`) or gradient-descent content from either file; do not modify any other session or project file. OUTPUT: Both files retain critical-point-finding and gradient-descent content but no longer duplicate the second-derivative classification; both link forward to Critical Points. VERIFY: `grep -c "critical_points" sessions/single_variable_calculus.md projects/single_variable_calculus/README.md` → both `>0`; `grep -c "f''(x_star) > 0" projects/single_variable_calculus/README.md` → `0`.

### Step 12.6: Deduplicate Partial Derivatives — forward-reference min/max/saddle classification

[x] Status

CONTEXT: `sessions/partial_derivatives_multivariate_calculus.md`'s Concept section previews "candidate minimum, maximum, or saddle" for a zero-gradient point, and `projects/partial_derivatives_multivariate_calculus/README.md`'s "Finding Max/Min via Partial Derivatives" section classifies `(0,0)` as a minimum by sampling nearby points — both are shallow versions of the Hessian/eigenvalue/Sylvester classification `sessions/critical_points.md` (Step 12.2) and its "Saddle or Bowl?" exercise now own. ACTION: In `sessions/partial_derivatives_multivariate_calculus.md`, trim the "candidate minimum, maximum, or saddle" paragraph to state only that a zero-gradient point is a critical point (dropping the classification preview), and add a forward reference to [Critical Points and the Second Derivative Test](critical_points.md) for how to classify it; in `projects/partial_derivatives_multivariate_calculus/README.md`, remove the entire "Finding Max/Min via Partial Derivatives" section and its two associated Help asserts (`grad_at_origin`/`samples` block), replacing the removed section with a one-line pointer to the "Saddle or Bowl?" exercise in `../critical_points/`; update the session's `## Exercise` section to drop the mention of finding a minimum via sampling. CONSTRAINTS: Do not remove the gradient/partial-derivative/chain-rule/Least-Squares content from either file; do not modify `## Finding the Best-Fit Line` or its Help content; do not modify any other session or project file. OUTPUT: Both files retain gradient/chain-rule/least-squares content but no longer duplicate min/max/saddle classification; both link forward to Critical Points. VERIFY: `grep -c "critical_points" sessions/partial_derivatives_multivariate_calculus.md projects/partial_derivatives_multivariate_calculus/README.md` → both `>0`; `grep -c "Finding Max/Min via Partial Derivatives" projects/partial_derivatives_multivariate_calculus/README.md` → `0`.
...
<!-- AI-GENERATED [claude-code:claude-sonnet-5]: Phase 13 (plan_history.md) -->

## Phase 13: Vector Calculus
...
### Step 13.2: Author sessions/vector_calculus.md — Divergence + Curl + Reference + Exercise

[x] Status

CONTEXT: `sessions/vector_calculus.md` (Step 13.1) has Motivation + Gradient but ends there; prompt_history.md's "Vector Calculus" section supplies the Divergence (`∇·F`) and Curl (`∇×F`, `3x3` determinant form) Purpose/Definition/Applications content verbatim, plus a `### Reference` instruction asking for the user-supplied video (`https://www.youtube.com/watch?v=lKXW7DRyyro`) plus a couple of additional highest-rated explanatory resources on vector calculus; `projects/vector_calculus/` does not exist yet (created next in Step 13.3). ACTION: Append a `### Divergence` subsection adapting the prompt's Purpose/Definition (`∇·F = ∂F_x/∂x + ∂F_y/∂y + ∂F_z/∂z`)/Applications content, with a "Paper Problem" sub-bullet contrasting `F(x,y) = (x, y)` (compute `∇·F = 2`, a source) against `F(x,y) = (-y, x)` (compute `∇·F = 0`, divergence-free); a `### Curl` subsection adapting the prompt's Purpose/Definition/Applications content, with a "Paper Problem" sub-bullet contrasting `F(x,y,0) = (-y, x, 0)` (compute curl `= (0,0,2)`, pure rotation) against `F(x,y,0) = (x, y, 0)` (compute curl `= 0`, irrotational); a `## Reference` section listing three items — the user-supplied video `https://www.youtube.com/watch?v=lKXW7DRyyro`; Khan Academy's Multivariable Calculus "Divergence and curl" unit (`https://www.khanacademy.org/math/multivariable-calculus`), a widely-used free explanatory resource for this exact topic; and the book *Div, Grad, Curl, and All That* by H. M. Schey, a classic, highly-rated informal text building physical intuition for vector calculus (cited by title/author, no link needed); a `## Exercise` section linking `../projects/vector_calculus/` and naming its three exercises ("Steepest Ascent," "Source or Sink?," "Does It Spin?"). CONSTRAINTS: Do not modify the Motivation or Gradient subsections from Step 13.1; do not touch `sessions/critical_points.md` or any other session file; do not fabricate a specific deep-link URL for the Khan Academy or book references beyond the stable pages/citation named above; the `## Exercise` section may name the three project exercises before `projects/vector_calculus/README.md` exists (created next in Step 13.3). OUTPUT: `sessions/vector_calculus.md` complete with Concept (all 4 subsections), a `## Reference` section listing 3 resources, and Exercise sections; no TODO markers. VERIFY: `grep -c "### Divergence\|### Curl" sessions/vector_calculus.md` → `2`; `grep -c "## Reference\|## Exercise" sessions/vector_calculus.md` → `2`; `grep -c "Khan Academy\|Schey" sessions/vector_calculus.md` → `2`; `grep -c "TODO" sessions/vector_calculus.md` → `0`.

### Step 13.3: Author projects/vector_calculus/README.md — three coding exercises + Help

[x] Status

CONTEXT: No `projects/vector_calculus/` exists; `sessions/vector_calculus.md` (Step 13.2) links `../projects/vector_calculus/` naming exercises "Steepest Ascent," "Source or Sink?," and "Does It Spin?"; `projects/critical_points/README.md`'s per-exercise (Skills/Paper Problem/Coding Exercise/Stretch goal) plus trailing `## Help` structure is the template to follow. ACTION: Create `projects/vector_calculus/README.md` with a `## Steepest Ascent` exercise: numerically approximate `∇f` for `f(x, y) = x^2 + y^2` via central differences over a grid, overlay the gradient vectors (quiver) on a heatmap/contour of `f`, and confirm they point radially outward; a `## Source or Sink?` exercise: numerically approximate `∇·F` via central differences for both `F(x,y) = (x, y)` and `F(x,y) = (-y, x)` over a grid, visualize each field (quiver) colored by local divergence, and confirm the sign/zero pattern matches the session's Paper Problem; a `## Does It Spin?` exercise: numerically approximate the 2D scalar curl (`∂F_y/∂x - ∂F_x/∂y`) via central differences for `F(x,y) = (-y, x)` and `F(x,y) = (x, y)`, visualize with `plt.streamplot`, and confirm which one spins; a trailing `## Help` section with a `plot_vector_field(X, Y, U, V, scalar=None)` helper (quiver, optionally colored/underlaid by a scalar field) and `assert np.allclose(...)` checks comparing each numerical result to the hand-derived closed-form values from the session's paper problems. CONSTRAINTS: Keep all three exercises toy/basic (2D only, small fixed grids); do not create a `solutions/` subdirectory; do not modify `sessions/vector_calculus.md` in this step. OUTPUT: New `projects/vector_calculus/README.md` with three named exercises and a Help section. VERIFY: `grep -c "## Help" projects/vector_calculus/README.md` → `1`; `grep -c "assert" projects/vector_calculus/README.md` → `>2`; `grep -c "## Steepest Ascent\|## Source or Sink?\|## Does It Spin?" projects/vector_calculus/README.md` → `3`.
...
<!-- AI-GENERATED [claude-code:claude-sonnet-5]: Phase 14 (plan_history.md) -->

## Phase 14: Vector Calculus Notation Fix
...
### Step 14.2: Convert matrix/vector notation in matrix_multiplication.md, systems_of_linear_equations.md, column_space_rank.md, orthogonality_projections.md

[x] Status

CONTEXT: an audit found these 4 sessions each use bare single-letter backtick matrix/vector symbols (`A`, `B`, `x`, `b`) in prose with no bold styling distinguishing them from scalars; every other non-calculus session (`scalars_vectors_matrices.md`, `linear_transformations.md`, `basis_change_of_basis.md`, `eigenvectors_eigenvalues.md`, and every AI-framing session) describes vectors/matrices entirely in prose with no bare symbols, so needs no edits; the accompanying `projects/*/README.md` files stay untouched (Python-identifier-tied). ACTION: apply the bold-matrix/bold-vector convention (`$\mathbf{A}$`/`$\mathbf{B}$` for matrices, `$\mathbf{x}$`/`$\mathbf{b}$` for vectors) to every prose mention of these symbols: `matrix_multiplication.md`'s opening paragraph (`matrix A`, `matrix B`); `systems_of_linear_equations.md`'s Concept and Exercise sections (`Ax = b`, `A`, `x`, `b`, throughout); `column_space_rank.md`'s Concept section (`b`, `Ax = b`, `A`, `x`); `orthogonality_projections.md`'s Concept section (`Ax = b`, `x`, `b`, `A`, `Ax`). Leave every `np.linalg.*(...)`, `A @ B`, `rot(30)` code-call/expression backticked exactly as-is. CONSTRAINTS: Do not touch any other prose in these 4 files; do not modify their matching `projects/*/README.md`; do not modify any other session file. OUTPUT: the 4 files' prose math mentions of A/B/x/b render as bold LaTeX; all code-call backticks unchanged. VERIFY: `grep -c '\mathbf{A}\|\mathbf{x}\|\mathbf{b}' sessions/systems_of_linear_equations.md sessions/column_space_rank.md sessions/orthogonality_projections.md` → all `>0`; `grep -c '\mathbf{A}\|\mathbf{B}' sessions/matrix_multiplication.md` → `>0`; `grep -c ' \`A\`\| \`B\`\| \`x\`\| \`b\`' sessions/matrix_multiplication.md sessions/systems_of_linear_equations.md sessions/column_space_rank.md sessions/orthogonality_projections.md` → all `0`.
...
### Step 14.4: Convert nabla/vector/Hessian notation in critical_points.md

[x] Status

CONTEXT: this is the most math-dense session — gradient `∇f(a)`, displacement vector `v`, unit direction `u`, point `a`, and the Hessian matrix `H` appear throughout the Multivariable subsection, the "Single Variable as n=1" subsection, and the Cheat Sheet table, in plain backtick/code-fence notation, plus 2 Mermaid diagrams whose node labels use plain `∇f(a)` text. ACTION: apply the convention — `$\vec{\nabla} f$` for the gradient, `$\mathbf{H}$` for the Hessian, bold for vector quantities (`$\mathbf{v}$` displacement, `$\mathbf{u}$` direction, `$\mathbf{a}$` the critical point) — throughout the Multivariable, "Single Variable as n=1", and Cheat Sheet subsections' prose, formula blocks, and table cells. Leave the Single Variable subsection (purely scalar: `f'(a)`, `f''(a)`, `h`) untouched. Leave the two Mermaid diagrams' node-label strings as plain text (Mermaid can't render LaTeX in labels); only update prose immediately around them if it references the now-bold symbols. CONSTRAINTS: Do not modify the Single Variable subsection, the Optimization Overview subsection, the Reference section, or the Exercise section; do not modify `projects/critical_points/README.md`; do not alter Mermaid diagram syntax or node labels. OUTPUT: the Multivariable/Cheat-Sheet content renders gradient/Hessian/vector quantities with LaTeX vector/bold styling; both Mermaid diagrams unchanged. VERIFY: `grep -c '\vec{\nabla}' sessions/critical_points.md` → `>0`; `grep -c '\mathbf{H}' sessions/critical_points.md` → `>0`; `grep -c '∇f(a)' sessions/critical_points.md` → `2` (only the 2 unchanged Mermaid node labels).
...
<!-- AI-GENERATED [claude-code:claude-sonnet-5]: Phase 15 (plan_history.md) -->

## Phase 15: Parameterization
...
### Step 15.9: Author sessions/parameterization.md

[ ] Status

CONTEXT: `sessions/parameterization.md` does not exist; `.tmp/parametrization_degrees_of_freedom_lesson.md` supplies the degrees-of-freedom framing (`(x,y)` unconstrained = 2 DOF vs. `y=x^2`-constrained = 1 DOF), curve/line/surface parameterization (`r(t)=p+t·u`, `g(t)=f(p+tu)`), and Paper Exercises 1-2; `.tmp/partial_derivatives_chain_rule.md`'s `f(x,y)=xy` constrained by `y=x^2` → `h(x)=x^3` example illustrates parameterization reducing degrees of freedom via the chain rule; `sessions/critical_points.md` is the structural/style template. ACTION: Create `sessions/parameterization.md`, title "Parameterization", `## Concept` with: a "Degrees of Freedom" subsection (the `(x,y)` vs. `y=x^2` framing, boxed definition "degrees of freedom = number of independent choices you can make"); a "Parameterizing Curves, Lines, and Surfaces" subsection (`r(t)=p+t·u`, `g(t)=f(p+tu)`); a "Constraint as Parameterization" subsection presenting the `f(x,y)=xy`/`y=x^2` → `h(x)=x^3` example, linking to `sessions/partial_derivatives_multivariate_calculus.md`'s `### Chain Rule` (Step 15.5); 2-3 Paper Problems adapted from `.tmp/parametrization_degrees_of_freedom_lesson.md` Exercises 1-2 (skip Exercise 3, Hessian-specific); a `## Reference` section citing both `.tmp` files' underlying ideas in prose; a `## Exercise` section linking `../projects/parameterization/`. CONSTRAINTS: Do not include any Hessian derivation, `uᵀHu`, or the "Connection to the Hessian" section/Exercise 3 — that content belongs exclusively in `sessions/critical_points.md` (Steps 15.12-15.13); do not modify any other session file. OUTPUT: New `sessions/parameterization.md` with Concept (3 subsections), Reference, Exercise sections; no Hessian content. VERIFY: `grep -c "Degrees of Freedom" sessions/parameterization.md` → `>0`; `grep -ic "hessian" sessions/parameterization.md` → `0`.
...
### Step 15.13: Enrich the Hessian derivation in critical_points.md's Multivariable subsection

[ ] Status

CONTEXT: `### Multivariable`'s existing "Where the Hessian comes from" and "Physical analogy" prose already derive `g''(0)=uᵀHu` via the directional-derivative chain rule and a position/velocity/acceleration table, but stop short of the boxed parameterized-Taylor-along-a-direction result that `.tmp/hessian.md` Part 3 and `.tmp/parametrization_degrees_of_freedom_lesson.md`'s "Connection to the Hessian" section derive: `f(a+tu)-f(a) = t·∇f(a)ᵀu + ½t²·uᵀH(a)u + ...`, reducing to `Δf ≈ ½uᵀH(a)u` at a critical point. ACTION: Extend "Where the Hessian comes from" with this boxed parameterized-Taylor formula, phrased using the `r(t)=p+t·u`/`g(t)=f(r(t))` parameterization language introduced in `sessions/parameterization.md` (Step 15.9), its critical-point reduction, and a one-sentence cross-link to `sessions/parameterization.md` for the general parameterization concept this derivation relies on; leave the existing "Physical analogy" table as-is (already covers position/velocity/acceleration). CONSTRAINTS: Do not modify `#### Saddle Points` (Step 15.12), `### Single Variable`, `### Optimization Overview`, the "Physical analogy" table, `## Reference`, or `## Exercise`; do not duplicate `sessions/parameterization.md`'s degrees-of-freedom material — link to it instead; this remains the sole canonical location in the repo for Hessian derivation content (per Step 15.9's exclusion). OUTPUT: `### Multivariable` includes the boxed parameterized-Taylor formula and its critical-point reduction, cross-linking `sessions/parameterization.md`. VERIFY: `grep -c "parameterization.md" sessions/critical_points.md` → `>0`; `grep -c "Delta f\|\\Delta f\|Δf" sessions/critical_points.md` → `>0`.

### Step 15.14: Add saddle-point and Hessian-direction project exercises to projects/critical_points/README.md

[ ] Status

CONTEXT: `projects/critical_points/README.md` has two exercises ("Finding the Bowl's Bottom" for single-variable, "Saddle or Bowl?" for multivariable) then `## Help`; Step 15.12's new single- and multi-variable `#### Saddle Points` subsections need a matching example-equation plot, and Step 15.13's enriched Hessian derivation warrants a hands-on directional-curvature exercise (`.tmp/parametrization_degrees_of_freedom_lesson.md` §6's "Explore a Surface One Direction at a Time" — computing `uᵀHu` for several directions, discovering extremal directions are the Hessian's eigenvectors). ACTION: Extend "Finding the Bowl's Bottom" with a `plt.plot` of Step 15.12's single-variable inflection example (`f(x)=x^3`) marking the saddle/inflection point; extend "Saddle or Bowl?" with a curvature surface/contour plot of Step 15.12's multivariable example (`f(x,y)=x^2-y^2`); add a new `## Which Way Does It Curve?` exercise (before `## Help`) implementing Step 15.13's directional-curvature idea: for a fixed `f(x,y)` and Hessian `H`, compute `uᵀHu` over 5+ unit directions `u`, plot curvature vs. direction angle, identify max/min-curvature directions; extend `## Help` with a `directional_curvature(H, angles)` helper and an `assert` confirming extremal directions align with `np.linalg.eigh(H)`'s eigenvectors. CONSTRAINTS: Do not remove or restructure existing Paper Problem/Coding Exercise/Stretch-goal content beyond the additive plot; keep the new exercise toy-scale (one fixed `H`, 2D only). OUTPUT: Both existing exercises gain their requested plot; new `## Which Way Does It Curve?` exercise and extended `## Help`. VERIFY: `grep -c "## Which Way Does It Curve?" projects/critical_points/README.md` → `1`; `grep -c "eigh" projects/critical_points/README.md` → `>0`.
